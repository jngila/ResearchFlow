...
export default function SupervisorNotificationsPage() {
  ...
  return (
    <DashboardShell
      ...
      title="Notifications"
      breadcrumbs={[{ label: 'Supervisor', href: '/dashboard/supervisor' }, { label: 'Notifications' }]}
    >
            ...
            {notifications.map(n => (
              <div key={n.id} className={cn('flex gap-4 px-5 py-4', !n.is_read && 'bg-blue-50/30')}>
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn('text-sm mb-0.5', n.is_read ? 'text-slate-500' : 'font-medium text-slate-800')}>{n.title}</p>
                  <p className="text-xs text-slate-400 leading-relaxed">{n.message}</p>
                  <p className="text-xs text-slate-300 mt-1">{formatDistanceToNow(new Date(n.created_at), { addSuffix: true })}</p>
                </div>
